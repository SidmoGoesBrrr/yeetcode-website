...
const QuickActions = ({ groupData, handleLogout }) => {
  ...
  // Remove all window.electronAPI and fetchRandomProblem logic
  // For all share buttons, just use window.open or alert as fallback
  // For random problem, just show a static modal or alert
  const handleRandomProblem = async () => {
    setLoadingProblem(true)

    try {
      // Simulate fetching a random problem
      ...
      const problem = {
        title: `Problem ${randomDifficulty} ${Math.floor(Math.random() * 100)}`,
        titleSlug: `problem-${randomDifficulty.toLowerCase()}-${Math.floor(Math.random() * 100)}`,
        difficulty: randomDifficulty,
        link: `https://leetcode.com/problems/${randomDifficulty.toLowerCase()}-${Math.floor(Math.random() * 100)}/`,
      }
  ...
  const handleConfirmProblem = async () => {
    if (selectedProblem) {
      const leetcodeUrl = selectedProblem.link
      if (window.electronAPI?.openExternalUrl) {
